...
EdgeBoxÇÃåvéZÅD
Ç±ÇøÇÁÇÇ‡Ç∆Ç…â¸ë¢
https://github.com/pdollar/edges/blob/master/private/edgeBoxesMex.cpp

*/


void EdgeBoxGenerator::setParams(option opt) {
	_alpha = opt.alpha;
	_beta = opt.beta;
	_eta = opt.eta;
	_minScore = opt.minScore;
	_maxBoxes = opt.maxBoxes;
	_edgeMinMag = opt.edgeMinMag;
	_edgeMergeThr = opt.edgeMergeThr;
	_clusterMinMag = opt.clusterMinMag;
	_maxAspectRatio = opt.maxAspectRatio;
	_minBoxArea = opt.minBoxArea;
	_gamma = opt.gamma;
	_kappa = opt.kappa;
}
...
void EdgeBoxGenerator::scoreAllBoxes(Boxes &boxes)
{
	...
	int arRad, scNum; float minSize = sqrt(_minBoxArea);
